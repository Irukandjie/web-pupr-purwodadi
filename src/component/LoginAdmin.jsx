...
const LoginAdmin = ({ onNavigate }) => {
  // Fungsi buat nanganin submit form
  const handleLogin = (e) => {
    e.preventDefault();
    
    // Nanti kalau udah ada API, lu bisa tambahin validasi di sini dulu (cek username/password).
    // Tapi untuk sementara, kita langsung arahin (bypass) ke Admin Dashboard pas tombol disubmit.
    
    if (onNavigate) {
      onNavigate('admin-dashboard'); // <-- Ini langsung nembak ke tampilan Dashboard[cite: 2]
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden font-sans">
      ...
      {/* --- Efek Background Estetik (Blur Merah & Emas) --- */}
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-[0_20px_50px_rgb(0,0,0,0.05)] border border-gray-100 p-8 md:p-10 relative z-10 mx-4">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-red-500/50 focus:border-red-500 outline-none transition-all"
                required
              />
            </div>
          </div>
          ...
          <div className="pt-2">
            <button 
              type="submit" 
              className="w-full py-3.5 bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-600/20 hover:bg-red-700 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 group"
            >
              Masuk ke Dashboard
        ...
        <div className="mt-8 text-center border-t border-gray-100 pt-6">
          <p className="text-sm text-gray-500 font-medium">
            ...
            <button 
              ...
              className="text-red-600 font-bold hover:text-amber-500 transition-colors outline-none"
            >
              Kembali ke Beranda
            </button>
          </p>
